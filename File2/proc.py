...
class ProcessFS:
    ...
    def open(self, filepath):
        ...
        if fcb == None:
            raise Exception("File {} Not Found in {}".format(filename, enclosingDir.name()))
        ...
        fileEntry = PerProcessFileEntry(fcb)
        descriptor = len(self.openFileTable)
        self.openFileTable.append(fileEntry)
    ...
    def close(self, descriptor):
        ...
        fileEntry = self.openFileTable[descriptor]
        fileEntry.fcb.decrOpenCount()
        if fileEntry.fcb.openCount == 0:
            self.openFileTable.remove(fileEntry)
            self.fs.sysOpenFileTable.remove(fileEntry)
            del fileEntry
    ...
    def read(self, descriptor, nBlocks=1):
        ...
        for i in range(nBlocks):
            newData.append(self.fs.readBlock(fcb.index[fileEntry.pos]))
            fileEntry.pos +=1
            fcb.updateAccessTime()
        if nBlocks == 1:
            return newData[0]
    ...
    def write(self, descriptor, data):
        ...
        fileEntry = self.openFileTable[descriptor]
        fcb = fileEntry.fcb
        newBlocks = self.fs.allocateBlocks(len(data))
        if newBlocks == None:
            raise Exception('Try to allocate {} blocks, only remains {} block.'.format(lackBlks,len(fs.freeBlockSet)))
        # save physical number
        fcb.index.extend(newBlocks)
        for d in data:
            self.fs.writeBlock(fcb.index[fileEntry.pos], d)
            fileEntry.pos +=1
            fcb.updateModTime()
